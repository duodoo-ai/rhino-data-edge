...
class DtuDataAggregate(models.Model):
    _name = 'dtu.data.aggregate'
    _description = '流量统计'

    name = fields.Char(string='流量统计', default='流量统计')
    ...
    def cron_compute_monthly_traffic(self):
        dtu_obj = self.env['dtu.data']
        aggregate_obj = self.env['dtu.data.aggregate']
        aggregate_record = aggregate_obj.search([('name', '=', '流量统计')])
        for record in dtu_obj.search([]):
            if record['time']:
